...
- **VTV Amplifier** (USA) and **Maroon Audio** (China) publish price ranges because their options change the price, so they use `priceFrom: true`. VTV's base price excludes its input-buffer, speaker-terminal and 12V-trigger options. VTV's 1ET400A model isn't tracked, as it's no longer made; its 1ET6525SA successor is. Maroon's base price is its Standard Edition; the Luxury Edition is $1,600, and shipping is $70.
...
- **SVS SB-1000 Pro subwoofer**: tracked in **Black Ash** everywhere, since the gloss finishes cost £100 more. Home AV Direct and Peter Tyson have a separate Black Ash product page. At Home Media, Premium Sound and Nintronics the monitor selects Black Ash first. Nintronics's page data names its colours only by variant ID, so Black Ash is matched by that ID (`44233925066985`). Not tracked, because their bot protection (Cloudflare) blocks the monitor: **Audio Affair** (even from a home connection) and **Hi-Fi Corner** (from GitHub). Check them with Visualping instead: https://www.audioaffair.co.uk/svs-sb1000-pro-subwoofer and https://www.hificorner.co.uk/product/svs-sb-1000-pro-subwoofer/?attribute_pa_finish=blackash (opens with Black Ash selected).
